...
- 相位差的产生同样需要左右两个 PD 像素，如今的绝大多数传感器为常规相位对焦，大部分为遮蔽式设计。人们在感光元件上预留出一些遮蔽像素点，专门用来进行相位检测。常见的方式有有单个像素遮一半（Half shield），或两个像素共用一个微透镜各自成像，有 Dual-PD、2×1 微透镜相位对焦（2×1 OCL）、2×2 微透镜相位对焦（2×2 OCL）等。

- 这些像素约占传感器总像素数的3%左右，用于检测相位后将不能用于成像，所以会影响画质。涵盖了从 IMX386 到 IMX600，从 IMX230 到 IMX608，从 IMX300 到 IMX400，从 IMX378 到 IMX380。
